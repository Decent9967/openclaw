...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - The running Gateway records its process identity and foreground or supervised mode in shared state. Restart preserves a verified live owner while it boots, even before its listener opens; a health timeout does not make that owner stale. A recorded foreground owner receives a targeted restart even when a native service is installed. Unverified listeners are reported instead of being killed. Gateways started by older versions retain their existing discovery path until restarted on the updated version.
